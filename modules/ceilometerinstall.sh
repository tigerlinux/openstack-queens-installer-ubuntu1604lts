...
then
	...
	# DEBIAN_FRONTEND=noninteractive aptitude -y install gnocchi-api python-gnocchiclient python-gnocchi gnocchi-metricd
	DEBIAN_FRONTEND=noninteractive aptitude -y install gnocchi-metricd python-gnocchiclient python-gnocchi
...
crudini --set /etc/ceilometer/ceilometer.conf DEFAULT aodh_is_enabled True
crudini --set /etc/ceilometer/ceilometer.conf api gnocchi_is_enabled True
crudini --set /etc/ceilometer/ceilometer.conf DEFAULT polling_namespaces central
...
then
	...
	echo "PATCH: Copying gnocchi API apache config"
	rm -f /etc/apache2/sites-enabled/gnocchi-api.conf
	cat ./libs/gnocchi/gnocchi-api.conf > /etc/apache2/sites-enabled/gnocchi-api.conf

...
# OpenStack QUEENS for Ubuntu 16.04lts
#
#
...
#
# We are not sure if this is still needed in QUEENS ubuntu packages, but just in case....
#
...
crudini --set /etc/neutron/neutron.conf DEFAULT network_auto_schedule True
crudini --set /etc/neutron/neutron.conf DEFAULT router_auto_schedule True
if [ $dvr_enable == "yes" ]
then
	crudini --set /etc/neutron/neutron.conf DEFAULT enable_dvr True
else
	crudini --set /etc/neutron/neutron.conf DEFAULT enable_dvr False
fi
crudini --set /etc/neutron/neutron.conf DEFAULT network_scheduler_driver neutron.scheduler.dhcp_agent_scheduler.WeightScheduler
crudini --set /etc/neutron/neutron.conf DEFAULT router_scheduler_driver neutron.scheduler.l3_agent_scheduler.LeastRoutersScheduler
crudini --set /etc/neutron/neutron.conf DEFAULT loadbalancer_pool_scheduler_driver neutron_lbaas.services.loadbalancer.agent_scheduler.ChanceScheduler
crudini --set /etc/neutron/neutron.conf quotas quota_driver neutron.db.quota.driver.DbQuotaDriver
...
fwaasversionstring="firewall"
case $fwaasversion in
"v1"|"V1"|"1")
        fwaasversionstring="firewall"
        ;;
"v2"|"V2"|"2")
        fwaasversionstring="firewall_v2"
        ;;
esac
 
if [ $vpnaasinstall == "yes" ]
then
	crudini --set /etc/neutron/neutron.conf DEFAULT service_plugins "neutron.services.l3_router.l3_router_plugin.L3RouterPlugin,neutron.services.qos.qos_plugin.QoSPlugin,$fwaasversionstring,neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPluginv2,vpnaas$thirdplugin"
else
	crudini --set /etc/neutron/neutron.conf DEFAULT service_plugins "neutron.services.l3_router.l3_router_plugin.L3RouterPlugin,neutron.services.qos.qos_plugin.QoSPlugin,$fwaasversionstring,neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPluginv2$thirdplugin"
fi
...
"v1"|"V1"|"1")
        ...
        crudini --set /etc/neutron/fwaas_driver.ini fwaas agent_version v1
        crudini --set /etc/neutron/fwaas_driver.ini fwaas conntrack_driver conntrack
...
crudini --set /etc/neutron/plugins/ml2/ml2_conf.ini agent extensions fip_qos
crudini --set /etc/neutron/plugins/ml2/ml2_conf.ini agent dscp 8
crudini --set /etc/neutron/plugins/ml2/ml2_conf.ini agent dscp_inherit true
...
then
        ...
        crudini --set /etc/neutron/plugins/ml2/ml2_conf.ini DEFAULT enable_distributed_routing True
else
        crudini --set /etc/neutron/plugins/ml2/ml2_conf.ini agent enable_distributed_routing False
        crudini --set /etc/neutron/plugins/ml2/ml2_conf.ini DEFAULT enable_distributed_routing False
fi
...
crudini --set /etc/neutron/metadata_agent.ini DEFAULT nova_metadata_host $novahost
...
crudini --set /etc/neutron/metadata_agent.ini DEFAULT endpoint_type adminURL

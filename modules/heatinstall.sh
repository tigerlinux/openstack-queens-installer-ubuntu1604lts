...
crudini --set /etc/heat/heat.conf DEFAULT region_name_for_services $endpointsregion

crudini --set /etc/heat/heat.conf DEFAULT default_software_config_transport POLL_SERVER_HEAT
crudini --set /etc/heat/heat.conf DEFAULT default_deployment_signal_transport HEAT_SIGNAL
...
crudini --set /etc/heat/heat.conf keystone_authtoken www_authenticate_uri http://$keystonehost:5000

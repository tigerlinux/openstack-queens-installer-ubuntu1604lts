...
#
# We install designate related packages and dependencies, non interactivelly of course
#
...
DEBIAN_FRONTEND=noninteractive aptitude -y install bind9
...
# DEBIAN_FRONTEND=noninteractive aptitude -y install designate designate-api designate-central \
# 	designate-common designate-doc designate-mdns designate-pool-manager designate-sink \
# 	designate-zone-manager python-designate

DEBIAN_FRONTEND=noninteractive aptitude -y install designate \
	python-designate \
...
cat ./libs/designate/named.conf.options > /etc/bind/named.conf.options

ln -s /etc/bind/rndc.key /etc/rndc.key

cat /etc/resolv.conf > /etc/resolv.conf.ORG

# chown bind.bind /etc/designate/rndc.key
...
#
# We silently stop all designate services, and disable some of them
#

stop designate-agent >/dev/null 2>&1
stop designate-api >/dev/null 2>&1
stop designate-central >/dev/null 2>&1
stop designate-mdns >/dev/null 2>&1
stop designate-pool-manager >/dev/null 2>&1
stop designate-zone-manager >/dev/null 2>&1
stop designate-sink >/dev/null 2>&1
stop designate-worker >/dev/null 2>&1
stop designate-producer >/dev/null 2>&1

systemctl stop designate-agent >/dev/null 2>&1
systemctl stop designate-api >/dev/null 2>&1
systemctl stop designate-central >/dev/null 2>&1
systemctl stop designate-mdns >/dev/null 2>&1
systemctl stop designate-pool-manager >/dev/null 2>&1
systemctl stop designate-zone-manager >/dev/null 2>&1
systemctl stop designate-sink >/dev/null 2>&1
systemctl stop designate-worker >/dev/null 2>&1
systemctl stop designate-producer >/dev/null 2>&1

# echo "manual" > /etc/init/designate-sink.override
# echo "manual" > /etc/init/designate-agent.override

systemctl disable designate-sink >/dev/null 2>&1
systemctl disable designate-agent >/dev/null 2>&1
systemctl disable designate-pool-manager >/dev/null 2>&1
systemctl disable designate-zone-manager >/dev/null 2>&1

killall -9 -u designate >/dev/null 2>&1


#
# And silently, we erase all logs too
#

rm -f /var/log/designate/*.log >/dev/null 2>&1
...
then
	...
	rm -f /etc/init/designate-sink.override
...
testdesignate=`dpkg -l designate-common 2>/dev/null|tail -n 1|grep -ci ^ii`
if [ $testdesignate == "0" ]
then
	echo ""

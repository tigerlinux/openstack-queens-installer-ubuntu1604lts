...
DEBIAN_FRONTEND=noninteractive aptitude -y purge virt-top ceilometer-agent-central ceilometer-agent-compute ceilometer-api \
	...
	nova-placement-api nova-common nova-compute nova-conductor nova-console nova-consoleauth \
...
rm -fr  /etc/qpid \
	...
	/etc/apache2/sites-available/wsgi-ceilometer.conf \
	/etc/apache2/sites-enabled/keystone-admin.conf \
	/etc/apache2/sites-enabled/gnocchi-api.conf \
...
rm -f /etc/apache2/sites-enabled/wsgi-*.conf
rm -f /etc/apache2/sites-available/wsgi-*.conf
...
systemclt restart apache2 2>/dev/null

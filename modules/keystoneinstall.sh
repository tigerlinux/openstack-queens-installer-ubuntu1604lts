...
#
# From OpenStack release 17 we'll use only fernet
#
...
crudini --set /etc/keystone/keystone.conf cache config_prefix cache.keystone
crudini --set /etc/keystone/keystone.conf cache distributed_lock = True

crudini --set /etc/keystone/keystone.conf revoke driver sql
crudini --set /etc/keystone/keystone.conf revoke expiration_buffer 1800
crudini --set /etc/keystone/keystone.conf revoke cache_time 3600

crudini --set /etc/keystone/keystone.conf token provider fernet
crudini --set /etc/keystone/keystone.conf token enforce_token_bind permissive
crudini --set /etc/keystone/keystone.conf token expiration 3600
crudini --set /etc/keystone/keystone.conf token cache_time 3600

crudini --set /etc/keystone/keystone.conf fernet_tokens key_repository "/etc/keystone/fernet-keys/"
crudini --set /etc/keystone/keystone.conf fernet_tokens max_active_keys 3

crudini --set /etc/keystone/keystone.conf identity driver sql
...
crudini --set /etc/keystone/keystone.conf resource cache_time 3600
crudini --set /etc/keystone/keystone.conf resource driver sql

crudini --set /etc/keystone/keystone.conf credential key_repository "/etc/keystone/credential-keys/"
crudini --set /etc/keystone/keystone.conf credential driver sql
crudini --set /etc/keystone/keystone.conf credential provider fernet
...
cat /usr/share/keystone/policy.v3cloudsample.json > /etc/keystone/policy.json
...
cp ./libs/keystone/keystone-admin.conf /etc/apache2/sites-enabled/keystone-admin.conf
...
sed -r -i "s/Listen\ 35357/Listen\ $keystonehost:35357/g" /etc/apache2/sites-enabled/keystone-admin.conf
...
#       --bootstrap-admin-url http://$keystonehost:5000/v3/ \
#        --bootstrap-internal-url http://$keystonehost:5000/v3/ \
...
# iptables -A INPUT -p tcp -m multiport --dports 5000,11211,5000 -j ACCEPT

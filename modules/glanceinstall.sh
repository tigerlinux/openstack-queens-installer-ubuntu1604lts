...
crudini --set /etc/glance/glance-api.conf keystone_authtoken www_authenticate_uri http://$keystonehost:5000
crudini --set /etc/glance/glance-api.conf keystone_authtoken auth_url http://$keystonehost:5000
...
crudini --set /etc/glance/glance-registry.conf keystone_authtoken www_authenticate_uri http://$keystonehost:5000
crudini --set /etc/glance/glance-registry.conf keystone_authtoken auth_url http://$keystonehost:5000
...
then
	...
	openstack image create "Cirros-0.4.0-i386" \
		--disk-format qcow2 \
		--public \
		--container-format bare \
		--project $keystoneadminuser \
		--protected \
		--file ./libs/cirros/cirros-0.4.0-i386-disk.img

	sync
	sleep 10
	sync

	openstack image create "Cirros-0.4.0-x86_64" \
		--disk-format qcow2 \
		--public \
		--container-format bare \
		--project $keystoneadminuser \
		--protected \
		--file ./libs/cirros/cirros-0.4.0-x86_64-disk.img
